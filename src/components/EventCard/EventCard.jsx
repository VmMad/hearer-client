...
const EventCard = ({ title, description, assistants, _id, loadEvents, assists, owner, imgSrc }) => {
    ...
    return (
        < Col xs={5} style={{ wordWrap: "break-word" }}
            ...
            {!assists ? <Button variant="warning" className="assistButton" onClick={() => assistEvent(_id, user._id)}>
                Asistiré</Button> : <Button disabled variant="warning">Ya asistes</Button>}
            {owner == user._id && <Button variant="warning" className="assistButton" onClick={() => navigate(`/event/${_id}/edit`)}>
                Editar</Button>}
        </Col >
    )
}
